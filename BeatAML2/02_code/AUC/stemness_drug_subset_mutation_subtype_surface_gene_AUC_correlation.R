...
# Outputs are written to 03_result/LSC17_AUC/stemness_drug_subset_mutation_subtype_surface_gene_auc.
...
output_dir <- file.path(project_root, "03_result", "LSC17_AUC", "stemness_drug_subset_mutation_subtype_surface_gene_auc")
...
target_presence <- auc_clean[
  ...
  .(
    n_auc_lsc17_expression_samples = uniqueN(Sample_id),
    n_auc_rows = .N
  ),
  by = inhibitor
][target_drugs_dt, on = "inhibitor"]
target_presence[, included_after_lsc17_expression_alignment := !is.na(n_auc_rows)]
target_presence[is.na(n_auc_lsc17_expression_samples), n_auc_lsc17_expression_samples := 0L]
target_presence[is.na(n_auc_rows), n_auc_rows := 0L]
data.table::fwrite(target_presence, file.path(output_dir, "target_drug_auc_presence_after_alignment.csv"))

missing_target_drugs <- target_presence[included_after_lsc17_expression_alignment == FALSE, inhibitor]
if (length(missing_target_drugs) > 0) {
  log_msg("Target drugs missing after LSC17/expression sample alignment: ", paste(missing_target_drugs, collapse = "; "))
}

auc_clean <- auc_clean[inhibitor %in% target_drugs]
if (nrow(auc_clean) == 0) {
  stop("No target drugs remain after LSC17/expression sample alignment.", call. = FALSE)
}
...
alignment_summary <- data.frame(
  item = c(
    ...
    "target_auc_rows_after_lsc17_expression_filter",
    "unique_target_auc_samples_after_filter",
    "target_drugs_requested",
    "target_drugs_after_filter",
    "surface_genes_used"
  ),
  value = c(
...
run_info <- data.frame(
  item = c(
    ...
    "output_dir",
    "target_drugs_requested",
    "target_drugs_after_lsc17_expression_alignment",
    "target_drugs_missing_after_lsc17_expression_alignment",
    "target_drug_list",
  ...
  value = c(
    ...
    output_dir,
    length(target_drugs),
    sum(target_presence$included_after_lsc17_expression_alignment),
...
analysis_definition <- data.frame(
  item = c(
    "interpretation",
    "auc_scope",
    "lsc17_scope",
  ...
  value = c(
    "Mutation-subtype-specific surface gene association with AUC combined with mutation-subtype-specific surface gene association with LSC17 score.",
    "AUC correlations are stratified by mutation subtype and restricted to target stemness-skewing resistance drugs.",
    "LSC17 correlations come from subtype_surface_latest and are restricted to mutation subtypes.",
    "FDR_lsc17 < 0.05, FDR_auc < 0.05, rho_lsc17 > 0, rho_auc > 0.",
    paste(target_drugs, collapse = "; ")
  )
)
data.table::fwrite(analysis_definition, file.path(output_dir, "analysis_definition.csv"))
