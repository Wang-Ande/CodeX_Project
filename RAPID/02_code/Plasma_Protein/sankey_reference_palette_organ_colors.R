...
if (is.na(script_file) || !nzchar(script_file)) {
  script_file <- file.path(getwd(), "02_code", "Plasma_Protein", "sankey_reference_palette_organ_colors.R")
}
...
out_dir <- file.path(repo_dir, "03_result", "Plasma_Protein", "Sankey_reference_palette_organ_colors")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

cat("Sankey reference-palette organ-color pipeline\n")
...
palette_variants <- list(
  list(
    id = "01_cell_fraction_reference_organ_colors",
    name = "Cell fraction reference with organ colors",
    note = "Reference-inspired abundance palette with direct organ-level colors on the left axis.",
...
readme_lines <- c(
  "# Sankey reference palette - organ colors",
  "",
  "This folder contains one reference-inspired color effect for the latest Sankey layout. The layout, data processing, axis alignment, labels, and flow alpha settings are unchanged from the final color-optimized version; only color assignments differ. The abundance colors follow the provided example figure: dark navy, mist blue, muted teal, wine red, and pale lavender. The left axis uses direct organ-level colors rather than shared organ-system colors.",
  "",
  "## Inputs",
  "",
  ...
  "## Palette folders",
  "",
  "- `01_cell_fraction_reference_organ_colors`: reference-style abundance palette with one color assigned directly to each requested organ on the left axis.",
  "",
  ...
  "- Organ modules are colored directly by organ; flows and Global/Enriched modules are colored by the abundance category.",
  "- The plotted label `NA` corresponds to the original `Not detected` category.",
  "",
  "Generated by `02_code/Plasma_Protein/sankey_reference_palette_organ_colors.R`."
)
