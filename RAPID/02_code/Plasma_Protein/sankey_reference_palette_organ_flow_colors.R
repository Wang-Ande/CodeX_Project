...
if (is.na(script_file) || !nzchar(script_file)) {
  script_file <- file.path(getwd(), "02_code", "Plasma_Protein", "sankey_reference_palette_organ_flow_colors.R")
}
...
out_dir <- file.path(repo_dir, "03_result", "Plasma_Protein", "Sankey_reference_palette_organ_flow_colors")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

cat("Sankey reference-palette organ-flow-color pipeline\n")
...
palette_variants <- list(
  list(
    id = "01_cell_fraction_reference_organ_flow_colors",
    name = "Cell fraction reference with organ flow colors",
    note = "Reference-inspired abundance palette with organ-colored flows from tissue-derived proteins to Global proteins.",
    abundance = c("Q4" = "#1E4267", "Q3" = "#789DB6", "Q2" = "#73B8AE", "Q1" = "#BFA8D8", "Not detected" = "#7A2E4D"),
    organs = c(
      "Brain" = "#D8C9EA",
      "Cerebellum" = "#8B315D",
      "Neuronal" = "#6A4C93",
...
make_organ_plot <- function(variant) {
  organ_plot_data <- organ_neat_counts %>%
    mutate(
      alluvium = row_number(),
      Flow_organ = as.character(.data$Organ)
    ) %>%
    select(alluvium, Organ, Neat_quantile, Flow_category, Flow_organ, aligned_display_weight)
  ...
  missing_color_organs <- setdiff(present_organs, names(variant$organs))
  if (length(missing_color_organs) > 0) {
    recycled_colors <- rep(unname(variant$organs), length.out = length(missing_color_organs))
    names(recycled_colors) <- missing_color_organs
    organ_colors <- c(variant$organs[present_organs[present_organs %in% names(variant$organs)]], recycled_colors)
  } else {
    organ_colors <- variant$organs[present_organs]
  }
  ...
  base_plot <- ggplot(
    ...
    geom_alluvium(
      aes(fill = Flow_organ, alpha = Flow_category),
      width = 0.13,
      knot.pos = 0.35,
      curve_type = "xspline",
      show.legend = FALSE
...
write_palette_key <- function(variant, variant_dir) {
  ...
  write.csv(
    bind_rows(
      ...
      organ_key %>% mutate(Color_group = "Organ")
    ),
