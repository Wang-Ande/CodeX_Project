...
if (is.na(script_file) || !nzchar(script_file)) {
  script_file <- file.path(getwd(), "02_code", "Plasma_Protein", "organ_to_neat_aligned_panel.R")
}
...
out_dir <- file.path(repo_dir, "03_result", "Plasma_Protein", "Organ_neat_aligned_panel")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

cat("Organ-to-neat aligned-panel pipeline\n")
...
base_plot <- ggplot(
  ...
  scale_x_continuous(
    breaks = c(1, 2),
    labels = c("Tissue", "Aligned neat quantile"),
    limits = c(0.30, 2.24),
    expand = c(0, 0)
  ) +
...
readme_lines <- c(
  "# Organ to neat aligned Sankey panel",
  "",
  "This folder contains an organ-side Sankey panel designed for compositing with the existing abundance Sankey.",
  "",
  ...
  "## Outputs",
  "",
  "- `organ_to_neat_aligned_panel.pdf`: standalone `Organ category -> aligned Neat quantile` Sankey plot. This is the PDF to place immediately left of the abundance Sankey.",
  ...
  "",
  "Generated by `02_code/Plasma_Protein/organ_to_neat_aligned_panel.R`."
)
